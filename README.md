...
The app allows for running extraction, transformation and export of data from fakturownia.pl account, all in an isolated docker environment. Fakturapp was designed to create docker image and container on top of it, every time it is run. After each run, successful or not, both image and container are deleted (in order to change that, edit src/Dockerfile).
